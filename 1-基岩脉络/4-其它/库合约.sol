...
// 不能接收以太币
// 不可以被销毁
// 需要注意的是，库合约中的函数可见性如果被设置为public或者external，则在调用这个函数时会触发一次delegatecall，此时会在调用合约的上下文中执行库合约的代码。
// 而如果被设置为internal，则不会触发。对于设置为private可见性的函数来说，其仅能在库合约中可见，在其他合约中不可见。
